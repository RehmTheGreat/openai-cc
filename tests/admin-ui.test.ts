...
test("Admin UI has one context control and no invented model limits", async () => {
  ...
  try {
    ...
    assert.match(f.html, /id="context-window" type="number" min="1"/); assert.match(f.html, /One Claude context window applies to every route/); assert.match(f.html, /JSON\.stringify\(\{contextWindow,routes\}\)/); assert.doesNotMatch(f.html, /route-context|ctx-'\+slot|Configured context:|API reported:|Discovered context:|max="1000000"/);
    ...
    assert.doesNotMatch(f.html, /Manual model ID/i); assert.doesNotMatch(f.html, /data-manual-model|saveManualModel/); assert.match(f.html, /Models are discovered automatically from credentials/); assert.match(f.html, /OpenAI-CC does not invent model names or context limits/); assert.match(f.html, /id="provider-tier"/); assert.match(f.html, /@media\(max-width:900px\)/); assert.match(f.html, /@media\(max-width:620px\)/);
...
test("Admin context is authoritative globally and Claude-facing names remain clean", async () => {
  const f = await fixture(); try {
    ...
    assert.equal(state.modelConfig.contextWindow, 1_050_000);
    for (const slot of ["default","fable","opus","sonnet","haiku"]) assert.equal("contextWindow" in state.modelConfig.routes[slot], false);
    const next = structuredClone(state.modelConfig); next.contextWindow = 1_234_567; next.routes.sonnet.vision = false; next.routes.sonnet.tools = false; next.routes.sonnet.reasoning = false;
    assert.equal((await mutate(f.base, f.csrf, "/admin/model-config", next)).status, 200);
    state = await (await fetch(`${f.base}/admin/state`)).json() as any; assert.equal(state.modelConfig.contextWindow, 1_234_567); assert.equal(state.routeHealth.sonnet.contextWindow, 1_234_567);
    const publicModels = await (await fetch(`${f.base}/v1/models`)).json() as any; assert.deepEqual(publicModels.data.map((m:any)=>m.id), ["default","fable","opus","sonnet","haiku"]); const sonnet = publicModels.data.find((m:any)=>m.id==="sonnet"); assert.equal(sonnet.max_input_tokens,1_234_567); assert.equal(sonnet.capabilities.image_input.supported,false); assert.equal(sonnet.capabilities.thinking.supported,false);
    for (const model of publicModels.data) assert.equal(model.max_input_tokens, 1_234_567);
...
test("Admin model discovery returns exactly what the configured discoverer reports and preserves errors", async () => {
  ...
  try {
    ...
    assert.match(f.html,/upstreamModelId/); assert.match(f.html,/Provider model ID/); assert.match(f.html,/discoveryErrors/);
  } finally { await close(f.server); }
});
