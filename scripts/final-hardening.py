...
def append_once(path: str, marker: str, text: str) -> None:
    ...
    if marker not in current:
        file.write_text(current + text, encoding="utf-8")
...
replace_once("src/admin/page.ts", "a.status.toUpperCase()", "statusLabel(a.status)")
replace_once(
    "src/admin/page.ts",
    ''''<div class="badges"><span class="badge '+a.status+'">'+statusLabel(a.status)+'</span>''',
    ''''<div class="badges"><span class="badge '+statusClass(a.status)+'">'+statusLabel(a.status)+'</span>''',
)
replace_once(
    "src/admin/page.ts",
