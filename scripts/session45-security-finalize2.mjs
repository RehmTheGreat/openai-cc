...
await edit('src/account-store.ts', s => {
  s = one(s, 'account.lastError = sanitizeError(message);', 'account.lastError = sanitizeError(message, account.apiKey ? [account.apiKey] : []);', 'rate-limit secret sanitizer');
  s = one(s, 'account.lastError = sanitizeError(message || "Authentication failed.");', 'account.lastError = sanitizeError(message || "Authentication failed.", account.apiKey ? [account.apiKey] : []);', 'auth secret sanitizer');
  s = one(s,
...
`function sanitizeError(value: string, exactSecrets: string[] = []): string {
  ...
  s = one(s, 'const PROVIDERS: ProviderKind[] = ["chatgpt", "zen", "nvidia", "google", "cloudflare"];\n', '', 'static provider list');
  return s;
});

await edit('src/provider-registry.ts', s => {
  s = one(s, 'import { OpenAICCError, conflict, notFound } from "./errors.js";', 'import { OpenAICCError, notFound } from "./errors.js";', 'unused conflict import');
  const old = 'safeDiscoveryError(responseText,response.status)';
  const count = s.split(old).length - 1;
  if (count !== 2) throw new Error(`Expected two discovery error calls, saw ${count}`);
  s = s.split(old).join('safeDiscoveryError(responseText,response.status,account.apiKey)');
  s = one(s,
