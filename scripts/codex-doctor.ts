...
try {
  const credentialId = args.credential ?? store.orderedReady("chatgpt")[0]?.id;
  ...
  const visibleModels = await boundary.listModels();
  const model = args.model ?? await configuredChatGptModel(dataDir, visibleModels);
  if (!model) throw new Error("Codex returned no GPT model suitable for the diagnostic probe.");
  ...
  await stage("1/4 model catalog", async () => {
    if (!visibleModels.includes(model)) {
      throw new Error(`${model} is not present in this account's Codex model catalog. Visible GPT models: ${visibleModels.filter((id) => id.startsWith("gpt-")).join(", ") || "<none>"}`);
    }
    console.log(`  ${model} is visible (${visibleModels.length} public Codex models).`);
  });

  await stage("2/4 direct Responses", async () => {
    const request = { model, stream: false, input: "Reply with exactly: openai-cc-direct-ok" };
    const response = await requireSuccessfulChatGptResponse(await boundary.responses(request), request);
    const payload = await response.json() as any;
    if (payload?.status !== "completed") throw new Error(`Codex returned HTTP 200 but response status was ${JSON.stringify(payload?.status ?? "<missing>")}.`);
    const inputTokens = Number(payload?.usage?.input_tokens ?? 0);
    const outputTokens = Number(payload?.usage?.output_tokens ?? 0);
    if (!(inputTokens > 0) || !(outputTokens > 0)) throw new Error(`Codex returned HTTP 200/completed but invalid usage: input=${inputTokens}, output=${outputTokens}.`);
    const outputTypes = Array.isArray(payload?.output) ? payload.output.map((item: any) => String(item?.type ?? "unknown")) : [];
    const text = outputText(payload);
    console.log(`  HTTP 200 completed; usage ${inputTokens}/${outputTokens}; output types ${outputTypes.join(", ") || "<none>"}.${text ? ` text=${JSON.stringify(text.slice(0, 120))}` : ""}`);
  });

  await stage("3/4 translated Claude request", async () => {
  ...
  console.log("\nPASS: ChatGPT OAuth, configured model, FCC translation, and Claude-style tools all reached Codex successfully.");
} catch (error: any) {
...
async function configuredChatGptModel(dataDir: string, visibleModels: string[]): Promise<string | undefined> {
  try {
    const raw = JSON.parse(await readFile(path.join(dataDir, "model-config.json"), "utf8")) as any;
    const routes = raw?.routes && typeof raw.routes === "object" ? Object.values(raw.routes) as any[] : [];
    const configured = routes.find((route) => route?.provider === "chatgpt" && typeof route?.model === "string" && visibleModels.includes(route.model));
    if (configured?.model) return configured.model;
  } catch {}
  return visibleModels.find((id) => id.startsWith("gpt-")) ?? visibleModels[0];
}
