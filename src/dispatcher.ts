...
import { ControlPlaneDispatcher, ControlPlaneOptions } from "./control-plane.js";
...
export interface DispatcherOptions extends ControlPlaneOptions {
  clientFactory?: (account: AccountRecord) => any;
}

/**
 * Canonical production dispatcher. /v1/messages owns all inference routing;
 * non-inference endpoints delegate to the control plane.
 *
 * ChatGPT is intentionally specialized: FCC translation -> raw
 * Evan/openai-oauth-compatible Codex transport -> /responses. Do not insert an
 * OpenAI SDK serializer into that path.
 */
export class Dispatcher {
  private readonly controlPlane: ControlPlaneDispatcher;
  ...
  constructor(
    ...
    const { clientFactory, ...controlPlaneOptions } = options;
    this.controlPlane = new ControlPlaneDispatcher(store, models, { ...controlPlaneOptions, providerRegistry: this.providers });
